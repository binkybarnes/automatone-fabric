...
public final class BuilderProcess extends BaritoneProcessHelper implements IBuilderProcess {
    ...
    public boolean placementPlausible(BlockPos pos, BlockState state) {
        ...
        return voxelshape.isEmpty() || ctx.world().doesNotIntersectEntities(null, voxelshape.move(pos.getX(), pos.getY(), pos.getZ()));
    }
    ...
    private Optional<Placement> possibleToPlace(BlockState toPlace, int x, int y, int z, BlockStateInterface bsi) {
        for (Direction against : Direction.values()) {
            ...
            for (Vec3 placementMultiplier : aabbSideMultipliers(against)) {
                ...
                Rotation rot = RotationUtils.calcRotationFromVec3d(RayTraceUtils.inferSneakingEyePosition(ctx.entity()), new Vec3(placeX, placeY, placeZ), ctx.entityRotations());
    ...
    private OptionalInt hasAnyItemThatWouldPlace(BlockState desired, HitResult result, Rotation rot) {
        if (!(ctx.entity() instanceof Player player)) return OptionalInt.empty();
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel, int recursions) {
        ...
        if (toPlace.isPresent() && isSafeToCancel && ctx.entity().isOnGround() && ticks <= 0) {
            ...
            baritone.getLookBehavior().updateTarget(rot, true);
            inventory.selected = toPlace.get().hotbarSelection;
            baritone.getInputOverrideHandler().setInputForceState(Input.SNEAK, true);
            if ((ctx.isLookingAt(toPlace.get().placeAgainst) && ((BlockHitResult) ctx.objectMouseOver()).getDirection().equals(toPlace.get().side)) || ctx.entityRotations().isReallyCloseTo(rot)) {
    ...
    private Goal placementGoal(BlockPos pos, BuilderCalculationContext bcc) {
        ...
        for (Direction facing : Movement.HORIZONTALS_BUT_ALSO_DOWN_____SO_EVERY_DIRECTION_EXCEPT_UP) {
            ...
            if (bcc.canPlaceAgainst(pos.relative(facing)) && placementPlausible(pos, bcc.getSchematic(pos.getX(), pos.getY(), pos.getZ(), current))) {
    ...
    private Goal breakGoal(BlockPos pos, BuilderCalculationContext bcc) {
        if (baritone.settings().goalBreakFromAbove.get() && bcc.bsi.get0(pos.above()).getBlock() instanceof AirBlock && bcc.bsi.get0(pos.above(2)).getBlock() instanceof AirBlock) { // TODO maybe possible without the up(2) check?
    ...
    private List<BlockState> approxPlaceable(int size) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            // <toxic cloud>
            BlockState placementState = ((BlockItem) stack.getItem()).getBlock().getStateForPlacement(new BlockPlaceContext(
                    new UseOnContext(
                            ctx.world(),
                            player,
                            InteractionHand.MAIN_HAND,
                            stack,
                            new BlockHitResult(
                                    new Vec3(ctx.entity().getX(), ctx.entity().getY(), ctx.entity().getZ()),
                                    Direction.UP,
                                    ctx.feetPos(),
                                    false
                            )
    ...
    private boolean valid(BlockState current, BlockState desired, boolean itemVerify) {
        ...
        if (current.getBlock() instanceof LiquidBlock && baritone.settings().okIfWater.get()) {
            return true;
        }
        if (current.isAir() && desired.isAir()) {
            return true;
        }
        if (current.isAir() && desired.is(baritone.settings().okIfAir.get())) {
            return true;
        }
        if (desired.isAir() && current.is(baritone.settings().buildIgnoreBlocks.get())) {
            return true;
        }
