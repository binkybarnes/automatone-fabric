    ...
    public void tick(boolean isLeftClick) {
        ...
        if (isLeftClick && isBlockTrace) {
            ...
            if (!Objects.equals(lastPos, pos)) {
                ctx.playerController().clickBlock(pos, ((BlockHitResult) trace).getDirection());
                ctx.entity().swingHand(InteractionHand.MAIN_HAND);
            }

            // Attempt to break the block
            if (ctx.playerController().onPlayerDamageBlock(pos, ((BlockHitResult) trace).getDirection())) {
                ctx.entity().swingHand(InteractionHand.MAIN_HAND);
            }
