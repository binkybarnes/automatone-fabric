...
public enum SchematicSystem implements ISchematicSystem {
    ...
    private final Registry<ISchematicFormat> registry = FabricRegistryBuilder.createSimple(ISchematicFormat.class, new ResourceLocation("automatone", "schematics")).buildAndRegister();

    SchematicSystem() {
        for (DefaultSchematicFormats s : DefaultSchematicFormats.values()) {
            Registry.register(this.registry, new ResourceLocation("automatone", s.name().toLowerCase(Locale.ROOT)), s);
        }
    }
