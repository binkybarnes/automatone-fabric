...
public class MovementAscend extends Movement {
    ...
    public static void cost(CalculationContext context, int x, int y, int z, int destX, int destZ, MutableMoveResult result) {
        ...
        if (jumpingToBottomSlab) {
            ...
            if (inLiquid) {
                ...
                walk = Math.max(JUMP_ONE_BLOCK_COST, WALK_ONE_BLOCK_COST / toPlace.getBlock().getSpeedFactor());
                walk += context.jumpPenalty;
            }
        }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (ctx.feetPos().equals(dest) || ctx.feetPos().equals(dest.add(getDirection().below()))) {
    ...
    public boolean headBonkClear() {
        BetterBlockPos startUp = src.up(Mth.ceil(ctx.entity().getHeight()));
        for (int i = 0; i < 4; i++) {
