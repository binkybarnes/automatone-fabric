...
public class MovementTraverse extends Movement {
        ...
        if (MovementHelper.canWalkOn(context.bsi, destX, y - 1, destZ, destOn, context.baritone.settings())) {//this is a walk, not a bridge
            ...
            if (!water) {
                if (destOn.getBlock() == Blocks.WATER) {
                    ...
                    WC = WALK_ONE_BLOCK_COST / destOn.getBlock().getSpeedFactor() / 2;
                }
                WC += WALK_ONE_BLOCK_COST / srcOnBlock.getSpeedFactor() / 2;
            }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (isTheBridgeBlockThere) {
            ...
            if (feet.getY() != dest.getY() && ladder && (destDown.is(BlockTags.CLIMBABLE))) {
            ...
            if (!collisionShape.isEmpty() && collisionShape.bounds().maxY < 1) { // see issue #118
            ...
            if (feet.equals(dest)) {
                ...
                Rotation backToFace = RotationUtils.calcRotationFromVec3d(ctx.headPos(), new Vec3(faceX, faceY, faceZ), ctx.entityRotations());
