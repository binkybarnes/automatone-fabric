...
public class MovementParkour extends Movement {
    ...
    public static void cost(CalculationContext context, int x, int y, int z, Direction dir, MutableMoveResult res) {
        ...
        int maxJump;
        if (standingOn.getBlock().getSpeedFactor() < 0.8) {  // arbitrary number, may need some tweaking
        ...
        for (int i = 2; i <= maxJump; i++) {
            ...
            if (!(landingOn.getBlock() instanceof FarmBlock) && MovementHelper.canWalkOn(context.bsi, destX, y - 1, destZ, landingOn, context.baritone.settings())) {
