...
public class MovementDescend extends Movement {
    ...
    public static void cost(CalculationContext context, int x, int y, int z, int destX, int destZ, MutableMoveResult res) {
        ...
        if (destDown.is(Blocks.SCAFFOLDING) && destDown.getValue(ScaffoldingBlock.BOTTOM)) {
        ...
        if (destUp.is(Blocks.SCAFFOLDING) && destUp.getValue(ScaffoldingBlock.BOTTOM)) {
            // same as above
            return;
        }
        ...
        double walk = waterModifier * (WALK_OFF_BLOCK_COST / fromDown.getBlock().getSpeedFactor());
    ...
    public boolean safeMode() {
        ...
        for (int y = 0; y <= 2; y++) { // we could hit any of the three blocks
            BlockState state = BlockStateInterface.get(ctx, into.above(y));
