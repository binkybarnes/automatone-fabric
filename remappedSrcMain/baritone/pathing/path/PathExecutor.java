...
public class PathExecutor implements IPathExecutor {
    ...
    public boolean onTick() {
        ...
        if (possiblyOffPath(status, MAX_DIST_FROM_PATH)) {
            ticksAway++;
            Automatone.LOGGER.warn("FAR AWAY FROM PATH FOR " + ticksAway + " TICKS. Current distance: " + status.getA() + ". Threshold: " + MAX_DIST_FROM_PATH);
    ...
    private boolean shouldSprintNextTick() {
        ...
        if (current instanceof MovementFall) {
            ...
            if (data != null) {
                ...
                clearKeys();
                behavior.baritone.getLookBehavior().updateTarget(RotationUtils.calcRotationFromVec3d(ctx.headPos(), data.getA(), ctx.entityRotations()), false);
